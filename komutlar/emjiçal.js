...
exports.run = async(client, message, args) => {
  ...
  if(args[1] === "tüm") {
    if(alınacakSunucu.emojis.cache.size < 2) return message.reply(`**${alınacakSunucu.name}** adlı sunucu 2'den emojiye sahip!`);
    try {
      await alınacakSunucu.emojis.forEach(x => message.guild.emojis.create(x.url, x.name));
