  ...
  var user = message.mentions.users.first() || message.author;
      message.channel.startTyping();
        ...
        Jimp.read(user.displayAvatarURL({format: "png"}), (err, image) => {
            ...
            image.greyscale()
            image.gaussian(3)
            Jimp.read("https://cdn.pixabay.com/photo/2013/07/13/12/32/tombstone-159792_960_720.png", (err, avatar) => {
                avatar.resize(100, 100)
                image.composite(avatar, 95, 159).write(`./img/wasted/${client.user.id}-${user.id}.png`);
                setTimeout(function() {
